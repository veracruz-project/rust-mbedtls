...
define!(
    ...
    //
    #[c_box_ty(mbedtls_ctr_drbg_context)]
    #[repr(C)]
    struct CtrDrbg {
        entropy: EntropyHolder,
    };
    const drop: fn(&mut Self) = mbedtls_ctr_drbg_free;
    impl<'a> Into<ptr> {}
);
...
impl CtrDrbg {
    ...
    pub fn new<T: EntropyCallback + 'static>(entropy: Arc<T>, additional_entropy: Option<&[u8]>) -> Result<Self> {
        let mut inner = Box::new(mbedtls_ctr_drbg_context::default());

        unsafe {
            mbedtls_ctr_drbg_init(&mut *inner);
    ...
    pub fn prediction_resistance(&self) -> bool {
        if self.inner.private_prediction_resistance == MBEDTLS_CTR_DRBG_PR_OFF {
            false
        } else {
            true
        }
