...
use mbedtls_sys::*;
use mbedtls_sys::{
    MBEDTLS_RSA_PKCS_V15 as RSA_PKCS_V15,
    MBEDTLS_RSA_PKCS_V21 as RSA_PKCS_V21,
    mbedtls_rsa_context as rsa_context,
    mbedtls_rsa_export as rsa_export,
    mbedtls_rsa_export_crt as rsa_export_crt,
    mbedtls_pk_info_from_type as pk_info_from_type,
};
...
impl Pk {
    ...
    pub fn generate_rsa<F: Random>(rng: &mut F, bits: u32, exponent: u32) -> Result<Pk> {
        let mut ret = Self::init();
        unsafe {
            mbedtls_pk_setup(&mut ret.inner, pk_info_from_type(Type::Rsa.into())).into_result()?;
            mbedtls_rsa_gen_key(
    ...
    pub fn generate_ec<F: Random, C: TryInto<EcGroup, Error = impl Into<Error>>>(rng: &mut F, curve: C) -> Result<Pk> {
        ...
        unsafe {
            ...
            mbedtls_pk_setup(&mut ret.inner, pk_info_from_type(Type::Eckey.into())).into_result()?;
            let ctx = ret.inner.private_pk_ctx as *mut mbedtls_ecp_keypair;
    ...
    pub fn custom_algo_id(&self) -> Result<&[u64]> {
        ...
        unsafe {
            let ctx = self.inner.private_pk_ctx as *const CustomPkContext;
            Ok(&(*ctx).algo_id)
        }
    }
        ...
        unsafe {
            match (Type::from(mbedtls_pk_get_type(&self.inner)), options) {
                ...
                | (Type::RsassaPss, Options::Rsa { padding }) => {
                    ...
                    mbedtls_rsa_set_padding(self.inner.private_pk_ctx as *mut rsa_context, padding, hash_id as _);
                }
    ...
    pub fn check_pair<F: Random>(public: &Self, private: &Self, rng: &mut F) -> bool {
        unsafe { mbedtls_pk_check_pair(&public.inner, &private.inner, Some(F::call), rng.data_ptr()) }
            .into_result()
            .is_ok()
    }
    ...
    getter!(
        /// Key length in bits
        len() -> usize = fn mbedtls_pk_get_bitlen
    );
    ...
    pub fn curve(&self) -> Result<EcGroupId> {
        ...
        unsafe { Ok((*(self.inner.private_pk_ctx as *const mbedtls_ecp_keypair)).private_grp.id.into()) }
    }
    ...
    pub fn ec_group(&self) -> Result<EcGroup> {
        ...
        match self.curve()? {
            EcGroupId::None => {
                ...
                unsafe {
                    let ecp = self.inner.private_pk_ctx as *const mbedtls_ecp_keypair;
    ...
    pub fn ec_public(&self) -> Result<EcPoint> {
        ...
        let q = &unsafe { (*(self.inner.private_pk_ctx as *const mbedtls_ecp_keypair)).private_Q };
        EcPoint::copy(q)
    }
    ...
    pub fn ec_private(&self) -> Result<Mpi> {
        ...
        let d = &unsafe { (*(self.inner.private_pk_ctx as *const mbedtls_ecp_keypair)).private_d };
        Mpi::copy(d)
    }
...
\0";
    ...
    fn rng() -> crate::rng::CtrDrbg {
        crate::rng::CtrDrbg::new(
            std::sync::Arc::new(crate::rng::OsEntropy::new()),
            None,
        ).unwrap()
    }
    ...
    #[test]
    fn parse_write_pem() {
        let parsed = Pk::from_private_key(&mut rng(), TEST_PEM.as_bytes(), None)
            .unwrap()
            .write_private_pem_string()
            .unwrap();
    ...
    #[test]
    fn parse_write_der() {
        let parsed = Pk::from_private_key(&mut rng(), TEST_DER, None)
            .unwrap()
    ...
    #[test]
    fn encrypt_v15_oaep() {
        let mut pk = Pk::from_private_key(&mut rng(), TEST_DER, None).unwrap();
    ...
    #[test]
    fn rsa_params() {
        let pk = Pk::from_private_key(&mut rng(), TEST_DER, None).unwrap();
