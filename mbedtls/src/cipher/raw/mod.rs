...
impl From<mbedtls_cipher_mode_t> for CipherMode {
    fn from(inner: mbedtls_cipher_mode_t) -> Self {
        match inner {
            ...
            MBEDTLS_MODE_KW => CipherMode::KW,
            MBEDTLS_MODE_KWP => CipherMode::KWP,
            // This should be replaced with TryFrom once it is stable.
...
define!(
    ...
    enum CipherType {
        ...
        Aes128Kw = MBEDTLS_CIPHER_AES_128_KW,
        Aes192Kw = MBEDTLS_CIPHER_AES_192_KW,
        Aes256Kw = MBEDTLS_CIPHER_AES_256_KW,
        Aes128Kwp = MBEDTLS_CIPHER_AES_128_KWP,
        Aes192Kwp = MBEDTLS_CIPHER_AES_192_KWP,
        Aes256Kwp = MBEDTLS_CIPHER_AES_256_KWP,
    }
);
...
impl Cipher {
    ...
    pub fn update_ad(&mut self, ad: &[u8]) -> Result<()> {
        unsafe { mbedtls_cipher_update_ad(&mut self.inner, ad.as_ptr(), ad.len()).into_result_discard() }
    }
