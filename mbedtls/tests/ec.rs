...
fn sign_verify() {
    ...
    let mut k = Pk::from_private_key(&mut rng, TEST_KEY_PEM.as_bytes(), None).unwrap();
    ...
    // Currently, default ECDSA is not randomized
...
fn verify_failure() {
    ...
    let len = k
        .sign(Sha256, data, &mut signature, &mut rng)
        .unwrap();
...
fn sign_verify_rfc6979_sig() {
    ...
    let mut rng = test_rng();
    let mut p256 = Pk::from_private_key(&mut rng, RFC6979_P256_KEY.as_bytes(), None).unwrap();
    ...
    let mut p521 = Pk::from_private_key(&mut rng, RFC6979_P521_KEY.as_bytes(), None).unwrap();
